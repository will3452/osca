...
>
    ...
    <div class="mb-6 {{ $errors->has('password') ? ' has-error' : '' }}" x-data="{isShow: false}">
        {{-- <div x-text="isShow"></div> --}}
        <label class="block font-bold mb-2" for="password">{{ __('Password') }}</label>
        <input class="form-control form-input form-input-bordered w-full" id="password" :type="isShow ? 'text': 'password'" name="password" required>
        <div class="text-right text-xs mt-2">
            <a href="javascipt:;" @click.prevent="isShow = ! isShow" x-text="isShow ? 'Hide Password': 'Show Password'" ></a>
        </div>
    </div>
